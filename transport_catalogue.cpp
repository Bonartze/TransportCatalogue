...
namespace RouteImitation {
    ...
    void TransportCatalogue::AddBusRouteStop(const std::string &number, const std::string &stop_name) {
        ...
        if (routes[number]->type == 'c')   // Use different formulas if route has cycle or uncycle type to count number of route's stops
            routes[number]->stops_number++;
        else {
            routes[number]->stops_number = routes[number]->routes_.size() * 2 + 1;
        }
        routes[number]->unique_stops.insert(stops[stop_name]); //for precise number of unique stops push it in unordered set
        if (!routes[number]->routes_.empty()) {        // pop last value from deque to calculate real distances and direct distances between two last stops
            auto last_stop = routes[number]->routes_.back();
            if (routes[number]->type == 'c')   // in dependence of type use different formulas  (1)
                routes[number]->route_length_direct += ComputeDistance(last_stop->coordinates,    //direct distances (without obstacles)
        ...
        if (routes[number]->route_length_direct != 0)                     //calculation a curvature of the whole path, check to not divide to zero
            routes[number]->
                    curvature =
